...
public final class ImbaExtension implements ModInitializer {
    ...
    public static final Item RETURN_TIMING_TOOL = new Item(new Item.Settings().maxCount(1));
    public static final Item PORTAL_ANIMATION_TOOL = new Item(new Item.Settings().maxCount(1));
    ...
    public void onInitialize() {
        ...
        Registry.register(Registries.ITEM, new Identifier("imba", "return_timing_tool"), RETURN_TIMING_TOOL);
        Registry.register(Registries.ITEM, new Identifier("imba", "portal_animation_tool"), PORTAL_ANIMATION_TOOL);

        RegistryKey<ItemGroup> group = RegistryKey.of(RegistryKeys.ITEM_GROUP, new Identifier("imba", "main"));
        ItemGroupEvents.modifyEntriesEvent(group).register(e -> {
            e.add(BLOCK_RULES_TOOL);
            ...
            e.add(OVERLAY_BARRIER);
            e.add(MASK_HITBOX_TOOL);
            e.add(RETURN_TIMING_TOOL);
            e.add(PORTAL_ANIMATION_TOOL);
        });
        ...
        MaskHitboxConfig.load();
        ReturnTimingConfig.load();
        PortalAnimationConfig.load();
        ...
        ReturnTimingNetworking.register();
        PortalAnimationNetworking.register();
        RoundRestoreToolHandler.register();
        OverlayBarrierToolHandler.register();
        DoorMaskCollisionHandler.register();

        AttackEntityCallback.EVENT.register((player, world, hand, entity, hit) ->
                !world.isClient && GameRoles.isHider(player) && GameManager.isCurrentParticipant(player)
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            MaskHitboxNetworking.sendSync(handler.getPlayer());
            ReturnTimingNetworking.sendSync(handler.getPlayer());
            PortalAnimationNetworking.sendSync(handler.getPlayer());
        });
    ...
    private static void registerCommands() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            dispatcher.register(net.minecraft.server.command.CommandManager.literal("imba_termination")
                    ...
                    .executes(ctx -> {
                        ForcedTerminationService.terminate(ctx.getSource().getServer());
                        return 1;
                    }));
            dispatcher.register(net.minecraft.server.command.CommandManager.literal("imba_reset_locations")
                    .requires(s -> s.hasPermissionLevel(2))
                    .executes(ctx -> {
                        var r = RoundRestoreConfig.restoreEnabled(ctx.getSource().getServer());
                        if (!r.anythingRestored()) {
                            ctx.getSource().sendFeedback(() -> Text.literal("§eНет сохранённых элементов для восстановления"), false);
                            return 0;
                        }
                        ctx.getSource().sendFeedback(() -> Text.literal("§aЛокации восстановлены §7• точек: §f" + r.points()
                                + " §7• слоёв: §f" + r.layers() + " §7• блоков всего: §f" + r.blocks()), true);
                        return 1;
                    }));
            dispatcher.register(net.minecraft.server.command.CommandManager.literal("imba_reset")
                    .requires(s -> s.hasPermissionLevel(2))
                    .then(net.minecraft.server.command.CommandManager.argument("player", GameProfileArgumentType.gameProfile())
                            .executes(ctx -> {
                                int count = 0;
                                for (var profile : GameProfileArgumentType.getProfileArgument(ctx, "player")) {
                                    var uuid = profile.getId();
                                    ServerPlayerEntity online = ctx.getSource().getServer().getPlayerManager().getPlayer(uuid);
                                    if (online != null) MaskService.resetMask(online); else MaskState.reset(uuid);
                                    count++;
                                }
                                final int n = count;
                                ctx.getSource().sendFeedback(() -> Text.literal("§aСброшено маскировок: §f" + n), true);
                                return n;
                            })));
            dispatcher.register(net.minecraft.server.command.CommandManager.literal("imba_start_game")
                    ...
                    .then(net.minecraft.server.command.CommandManager.argument("location", StringArgumentType.greedyString())
                            .suggests((ctx, builder) -> {
                                String needle = normalizeLocation(builder.getRemaining().replace("\"", "").replace("«", "").replace("»", ""));
                                for (int i = 0; i < GameConfig.ROUNDS.size(); i++) {
                                    String name = GameConfig.getLocationName(i), id = GameConfig.ROUNDS.get(i).id;
                                    if (needle.isEmpty() || normalizeLocation(name).contains(needle) || normalizeLocation(id).contains(needle))
                                        builder.suggest(name.contains(" ") ? "\"" + name.replace("\"", "\\\"") + "\"" : name);
                                }
                                return builder.buildFuture();
                            })
                            .executes(ctx -> {
                                String raw = StringArgumentType.getString(ctx, "location");
                                int found = findLocationIndex(raw);
                                if (found == -2) {
                                    ctx.getSource().sendError(Text.literal("Найдено несколько локаций. Уточните название: " + stripOuterQuotes(raw).trim()));
                                    return 0;
                                }
                                if (found < 0) {
    ...
    private static int findLocationIndex(String raw) {
        String requested = normalizeLocation(raw);
        if (requested.isEmpty()) return -1;
        List<Integer> partial = new ArrayList<>();
        for (int i = 0; i < GameConfig.ROUNDS.size(); i++) {
            var round = GameConfig.ROUNDS.get(i);
            String configured = normalizeLocation(GameConfig.getLocationName(i));
            String rawName = normalizeLocation(round.locationName);
            String id = normalizeLocation(round.id);
            if (requested.equals(configured) || requested.equals(rawName) || requested.equals(id)
                    || requested.equals(Integer.toString(i + 1))) return i;
            if (configured.contains(requested) || rawName.contains(requested) || id.contains(requested)) partial.add(i);
        }
        return partial.size() == 1 ? partial.get(0) : partial.size() > 1 ? -2 : -1;
    }

    private static String normalizeLocation(String value) {
        String clean = stripOuterQuotes(value == null ? "" : value).replaceAll("§.", "");
        clean = Normalizer.normalize(clean, Normalizer.Form.NFKC).trim().replaceAll("\\s+", " ");
    ...
    private static String stripOuterQuotes(String value) {
        ...
        if (clean.length() >= 2) {
            char a = clean.charAt(0), b = clean.charAt(clean.length() - 1);
            if ((a == '\"' && b == '\"') || (a == '\'' && b == '\'') || (a == '«' && b == '»') || (a == '“' && b == '”'))
