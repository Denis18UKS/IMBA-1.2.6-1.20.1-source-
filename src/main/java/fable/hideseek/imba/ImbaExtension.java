...
public final class ImbaExtension implements ModInitializer {
    ...
    public static final Item PANEL_HITBOX_TOOL = new Item(new Item.Settings().maxCount(1));
    ...
    public static final Item OVERLAY_BARRIER_TOOL = new Item(new Item.Settings().maxCount(1));
    ...
    public void onInitialize() {
        ...
        Registry.register(Registries.ITEM, new Identifier("imba", "panel_hitbox_tool"), PANEL_HITBOX_TOOL);
        ...
        Registry.register(Registries.ITEM, new Identifier("imba", "overlay_barrier_tool"), OVERLAY_BARRIER_TOOL);
        ...
        ItemGroupEvents.modifyEntriesEvent(group).register(entries -> {
            ...
            entries.add(PANEL_HITBOX_TOOL);
            entries.add(AIR_FIXATION_TOOL);
            entries.add(MESSAGE_SETTINGS_TOOL);
            entries.add(MASK_HITBOX_TOOL);
            entries.add(OVERLAY_BARRIER_TOOL);
        });
        ...
        PanelHitboxNetworking.register();
        MessageSettingsNetworking.register();
        MaskHitboxNetworking.register();
        OverlayBarrierNetworking.register();
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            HologramNetworking.sendSync(player);
            PanelSettingsNetworking.sendSync(player);
            MaskHitboxNetworking.sendSync(player);
            OverlayBarrierNetworking.sendSync(player);
        });
        registerCommands();
    }
                    ...
                    .then(net.minecraft.server.command.CommandManager.argument("player", GameProfileArgumentType.gameProfile())
                            .executes(ctx -> {
                                ...
                                for (var profile : GameProfileArgumentType.getProfileArgument(ctx, "player")) {
                                    ...
                                    if (online != null) MaskService.resetMask(online);
                                    else MaskState.reset(uuid);
                                    count++;
                                }
                                final int total = count;
                                ctx.getSource().sendFeedback(() -> Text.literal("§aСброшено маскировок: §f" + total), true);
                                return total;
                            })));
            ...
            dispatcher.register(net.minecraft.server.command.CommandManager.literal("imba_start_game")
                    ...
                    .then(net.minecraft.server.command.CommandManager.argument("location", StringArgumentType.greedyString())
                            .suggests((ctx, builder) -> {
                                String needle = normalizeLocation(builder.getRemaining()
                                        .replace("\"", "").replace("«", "").replace("»", ""));
                                for (int i = 0; i < GameConfig.ROUNDS.size(); i++) {
                                    String name = GameConfig.getLocationName(i);
                                    String id = GameConfig.ROUNDS.get(i).id;
                                    if (needle.isEmpty() || normalizeLocation(name).contains(needle)
                                            || normalizeLocation(id).contains(needle)) {
                                        builder.suggest(name.contains(" ")
                                                ? "\"" + name.replace("\"", "\\\"") + "\""
                                                : name);
                                    }
                                }
                            ...
                            .executes(ctx -> {
                                ...
                                if (found == -2) {
                                    ctx.getSource().sendError(Text.literal(
                                            "Найдено несколько локаций. Уточните название: "
                                                    + stripOuterQuotes(raw).trim()));
                                    return 0;
                                }
                                if (found < 0) {
                                    ctx.getSource().sendError(Text.literal(
    ...
    private static int findLocationIndex(String raw) {
        ...
        for (int i = 0; i < GameConfig.ROUNDS.size(); i++) {
            ...
            if (requested.equals(configured) || requested.equals(rawName)
                    || requested.equals(id) || requested.equals(Integer.toString(i + 1))) return i;
