...
public final class HologramNetworking {
    ...
    public static void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(SAVE,(server,player,handler,buf,sender)->{
            ...
            server.execute(()->{
                if(!player.hasPermissionLevel(2)||location<0||location>=GameConfig.ROUNDS.size()||Identifier.tryParse(world)==null||!finite(x,y,z,yaw,scale,contrast))return;
                HologramConfig.saveProjector(id,location,world,x,y,z,yaw,scale,light,textBackground,contrast);
    ...
    private static boolean finite(double x,double y,double z,float yaw,float scale,float contrast){return Double.isFinite(x)&&Double.isFinite(y)&&Double.isFinite(z)&&Float.isFinite(yaw)&&Float.isFinite(scale)&&Float.isFinite(contrast)&&Math.abs(x)<=3e7&&Math.abs(y)<=2048&&Math.abs(z)<=3e7&&scale>0&&scale<=8&&contrast>=.5f&&contrast<=2.0f;}
    ...
    public static void sendSync(ServerPlayerEntity player){var list=HologramConfig.snapshot();PacketByteBuf out=PacketByteBufs.create();out.writeVarInt(list.size());for(var p:list){out.writeInt(p.id);out.writeVarInt(p.location);out.writeString(p.world,128);out.writeDouble(p.x);out.writeDouble(p.y);out.writeDouble(p.z);out.writeFloat(p.yaw);out.writeFloat(p.scale);out.writeByte(p.light);out.writeBoolean(p.textBackground);out.writeFloat(p.contrast);}ServerPlayNetworking.send(player,SYNC,out);}
}
