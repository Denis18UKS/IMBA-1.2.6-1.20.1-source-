...
public final class LocalNightClientData {
    ...
    public static void registerNetworking() {
        ClientPlayNetworking.registerGlobalReceiver(LocalNightNetworking.SYNC, (client, handler, buf, sender) -> {
            ...
            for (int i = 0; i < count; i++) next.add(new Anchor(buf.readString(128), buf.readDouble(), buf.readDouble()));
            client.execute(() -> {
    ...
    public static int selectedLocation() { return selectedLocation; }
    ...
    public static int nearestLocation(String world, double x, double z) {
        ...
        for (int i = 0; i < anchors.size(); i++) {
            ...
            double dx = x - anchor.x, dz = z - anchor.z;
    ...
    public static long visualTime() {
        ...
        if (nearest < 0 || selectedLocation != nearest) return DAY_TIME;
        return isSnowVillageStreet(PanelData.locationName(nearest)) ? NIGHT_TIME : DAY_TIME;
    }

    public static boolean isSnowVillageStreet(String value) {
