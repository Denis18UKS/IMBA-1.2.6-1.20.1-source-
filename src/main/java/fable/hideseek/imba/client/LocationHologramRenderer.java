...
/** Location photo hologram renderer. */
public final class LocationHologramRenderer {
    ...
    public static void register() {
        WorldRenderEvents.AFTER_ENTITIES.register(ctx -> {
            ...
            for (var projector : HologramClientData.snapshot()) {
                ...
                float half = (BASE_SIZE * scale) / 2.0F;
                ...
                if (isViewerOnFrontSide(projector, camera.x, camera.z)) drawTitle(client, ctx, matrices, projector, half, scale);
                matrices.pop();
            }
        });
    }
    ...
    private static boolean isViewerOnFrontSide(HologramClientData.Projector projector, double cameraX, double cameraZ) {
        double yawRad = Math.toRadians(projector.yaw());
        double frontX = -Math.sin(yawRad), frontZ = Math.cos(yawRad);
        return (cameraX - projector.x()) * frontX + (cameraZ - projector.z()) * frontZ >= 0.0D;
    }

    private static void drawTitle(MinecraftClient client,
                                  net.fabricmc.fabric.api.client.rendering.v1.WorldRenderContext ctx,
                                  MatrixStack matrices, HologramClientData.Projector projector,
                                  float half, float scale) {
        String title = PanelData.locationName(projector.location());
        if (title == null || title.isBlank()) return;
        String[] lines = splitTitle(title, projector.titleBreak());
        matrices.push();
        matrices.translate(0.0F, -half - 0.115F, 0.012F);
        float configuredTextScale = Math.max(0.50F, Math.min(3.0F, projector.textScale()));
        float textScale = 0.0145F * Math.max(0.88F, scale) * configuredTextScale;
        matrices.scale(textScale, -textScale, textScale);
        int background = projector.textBackground() ? 0x66000000 : 0;
        for (int line = 0; line < lines.length; line++) {
            String value = lines[line];
            float width = client.textRenderer.getWidth(value);
            client.textRenderer.draw(value, -width / 2.0F, line * (client.textRenderer.fontHeight + 2.0F), 0xFFFFFFFF,
                    false, matrices.peek().getPositionMatrix(), ctx.consumers(), TextRenderer.TextLayerType.NORMAL,
                    background, LightmapTextureManager.MAX_LIGHT_COORDINATE);
        }
        matrices.pop();
    }

    static String[] splitTitle(String title, int titleBreak) {
        if (title == null) return new String[]{""};
        if (titleBreak <= 0 || titleBreak >= title.length()) return new String[]{title};
        String first = title.substring(0, titleBreak).trim();
        String second = title.substring(titleBreak).trim();
        if (first.isEmpty() || second.isEmpty()) return new String[]{title};
        return new String[]{first, second};
    }

    private static void vertex(VertexConsumer vertices, MatrixStack.Entry entry,
                               float x, float y, float z, float u, float v) {
        vertices.vertex(entry.getPositionMatrix(), x, y, z).color(255, 255, 255, 255).texture(u, v)
