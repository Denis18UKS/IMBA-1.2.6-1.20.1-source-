...
public final class WorldPanelRenderer {
    ...
    private static void renderSettings(MatrixStack matrices, VertexConsumerProvider consumers, Anchor anchor,
        ...
        drawScaledCentered(matrices, consumers, client.textRenderer,
                PanelData.heartsLabel == null || PanelData.heartsLabel.isBlank() ? "Сердца" : PanelData.heartsLabel,
                heartsX, -30, 58.0F, 0xFFFFAA00);
        ...
        drawCenteredAtScale(matrices, consumers, client.textRenderer,
                "Shift+ПКМ — настройки", 0, 44, 0.62F, 0xFFAAAAAA);
        matrices.pop();
    }
    ...
    private static void drawScaledCentered(MatrixStack matrices, VertexConsumerProvider consumers,
                                           TextRenderer renderer, String text, float centerX, float y,
                                           float maximumWidth, int color) {
        String value = text == null ? "" : text;
        float width = Math.max(1.0F, renderer.getWidth(value));
        drawCenteredAtScale(matrices, consumers, renderer, value, centerX, y,
                Math.min(1.0F, maximumWidth / width), color);
    }
}
