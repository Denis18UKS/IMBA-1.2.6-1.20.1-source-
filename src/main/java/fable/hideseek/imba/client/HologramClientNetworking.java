...
public final class HologramClientNetworking {
    private HologramClientNetworking() {}

    public static void register() {
        ClientPlayNetworking.registerGlobalReceiver(HologramNetworking.SYNC, (client, handler, buf, sender) -> {
            int n = Math.max(0, Math.min(buf.readVarInt(), 4096));
