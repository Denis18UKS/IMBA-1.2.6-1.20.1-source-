...
public final class MaskBlockConfigScreen extends Screen {
    ...
    public MaskBlockConfigScreen() {
        super(Text.literal("Настройка блоков масок"));
    ...
    private void clampScroll() {
        scroll = MathHelper.clamp(scroll, 0, Math.max(0, filtered.size() - visibleRows()));
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            if (mouseX >= rowLeft && mouseX < rowRight && mouseY >= y && mouseY < y + 21) {
                bg = 0x77606060;
            }
            ...
            context.fill(statusLeft, y + 2, rowRight - 2, y + 19,
                    full ? 0xAA1E6B2D : 0xAA7A5520);
            context.drawCenteredTextWithShadow(textRenderer,
                    full ? "ПОЛНОЦЕННЫЙ" : "НЕПОЛНЫЙ",
                    statusLeft + (statusWidth - 2) / 2,
                    y + 7,
                    full ? 0xAAFFAA : 0xFFFF55);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (newFull) {
            nonFullBlocks.remove(id);
        } else {
            nonFullBlocks.add(id);
        }
