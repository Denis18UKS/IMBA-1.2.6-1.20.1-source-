...
public final class MaskCollisionShapes {
    ...
    public static List<Box> create(MaskType type, net.minecraft.block.Block block, float rotation,
            boolean doorOpen, double x, double y, double z) {
        return create(type, block, rotation, doorOpen, x, y, z, null);
    }

    public static List<Box> create(MaskType type, net.minecraft.block.Block block, float rotation,
            boolean doorOpen, double x, double y, double z, MaskHitboxConfig.Bounds override) {
    ...
    public static Vec3d nearestHorizontalSeparation(Box mover, Box obstacle) {
        ...
        double toLeft = mover.maxX - obstacle.minX, toRight = obstacle.maxX - mover.minX,
                toNorth = mover.maxZ - obstacle.minZ, toSouth = obstacle.maxZ - mover.minZ;
        double best = Double.POSITIVE_INFINITY, dx = 0, dz = 0;
        if (toLeft > 0 && toLeft < best) { best = toLeft; dx = -toLeft; dz = 0; }
        if (toRight > 0 && toRight < best) { best = toRight; dx = toRight; dz = 0; }
        if (toNorth > 0 && toNorth < best) { best = toNorth; dx = 0; dz = -toNorth; }
        if (toSouth > 0 && toSouth < best) { dx = 0; dz = toSouth; }
